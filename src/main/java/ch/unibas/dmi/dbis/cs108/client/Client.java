 ...
 * TCP/IP network. It handles sending and receiving messages according to a defined protocol using
 * {@link ProtocolReaderClient} and {@link ProtocolWriterClient}. The client reads user's input and
 * allows the user to interact with a multiplayer game server by sending commands such as creating
...
public class Client {
  ...
  /**
   * Starts the client, connects to the server, and begins processing user input. A new thread is
   * created to handle incoming server messages. The main thread handles user input and sends the
   * corresponding commands via the {@link ProtocolWriterClient} to the server.
   */
  public void start() {
    try {
      ...
      try {
        Thread.sleep(
            2000); //Welcomemessage comes before the System.out.println's that come after that.
      ...
      ProtocolWriterClient protocolClient = new ProtocolWriterClient(
          out);  // Methodenimplementation im WriterClient
      ...
      while (true) {
        ...
        if (line.equalsIgnoreCase("leave")) {
          ...
          if (line.length() <= 8) {
            System.out.println(
                "Please provide a receiver and message. Usage: whisper <user> <message>");
            continue;
          }
          ...
          if (username != null && protocolReader.bike) {
            ...
            System.out.println(
                "Error: You must select a bike before getting ready. Use 'selectbike <color>' command.");
          }
  ...
  /**
   * Sets the {@link WelcomeLobbyController} for this client by forwarding it to the
   * {@link ProtocolReaderClient} and the {@link ProtocolWriterClient}. This allows the
   * {@link ProtocolReaderClient} to update the GUI with incoming messages.
   *
  ...
  /**
   * Sets the {@link GameLobbyController} for this client by forwarding it to the
   * {@link ProtocolReaderClient} and the {@link ProtocolWriterClient}. This allows the
   * {@link ProtocolReaderClient} to update the GameLobby GUI with incoming messages.
   *
  ...
  /**
   * Returns the {@link ProtocolReaderClient} used by this client. The {@link ProtocolReaderClient}
  ...
  /**
   * Returns the {@link ProtocolWriterClient} used by this client. The {@link ProtocolWriterClient}
   * is responsible for sending messages to the server.
   *
