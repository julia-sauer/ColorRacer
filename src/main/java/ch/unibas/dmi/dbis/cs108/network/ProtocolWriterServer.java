...
/**
 * The {@code ProtocolWriterServer} class is responsible for sending protocol messages from the
 * server to connected clients. It supports broadcasting messages, sending individual commands, and
 * whispering to specific clients. All messages are encoded in UTF-8 to ensure cross-platform
 * compatibility.
 */
public class ProtocolWriterServer {

  /**
   * A list of writers connected to all clients. This is used for broadcasting messages to all
   * clients.
   */
  ...
  /**
   * This method sends an already formatted string message to the client, more specifically to the
   * {@link ProtocolReaderClient}.
   *
  ...
  /**
   * This method sends a private (whisper) message to a specific user using the {@code WISP}
   * command.
   *
