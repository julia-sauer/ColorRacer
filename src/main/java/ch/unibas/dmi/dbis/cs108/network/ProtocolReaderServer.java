...
//                        }
        ...
        case FNSH: {
          ...
          if (userLobby == null || userLobby.getLobbyName().equalsIgnoreCase("Welcome")) {
            protocolWriterServer.sendInfo(
                "You are not currently in a lobby or in the Welcome lobby");
            break;
          }
          if (username.trim().equalsIgnoreCase(userLobby.getHostName().trim())
              || !userLobby.winners.isEmpty()) {
          ...
          protocolWriterServer.sendInfo(
              "You are not the host of the lobby and cannot end the game.");
          break;
        }
        default:
