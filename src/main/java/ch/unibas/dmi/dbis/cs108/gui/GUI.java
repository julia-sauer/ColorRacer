...
/**
 * The {@code GUI} class is a JavaFX Application that loads the chat interface defined in the FXML
 * file ChatTemplate and connects it to the client. This class is responsible for launching the
 * graphical user interface, injecting the necessary protocol objects into the
 * {@link WelcomeLobbyController}, and displaying the chat window.
 *
 * @author julia
 */
public class GUI extends Application {
  ...
  /**
   * This method loads the FXML layout for the chat interface,retrieves the
   * {@link WelcomeLobbyController}, injects the
   * {@link ch.unibas.dmi.dbis.cs108.network.ProtocolWriterClient} and
   * {@link WelcomeLobbyController} references from the client, and displays the chat window. It
   * assumes that the FXML file is located in the resource folder at {@code /ChatTemplate.fxml}.
   *
