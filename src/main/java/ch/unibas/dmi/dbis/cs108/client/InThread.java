...
class InThread implements Runnable {
  ...
  /**
   * Reads data from the InputStream and writes it to the standard output. This method runs in a
   * loop and continuously reads bytes from the InputStream until the end of the stream is reached
   * (indicated by a return value of -1 from read()).
   */
  public void run() {
